...
function gradesAvg(arr) {
    ...
    arr.map (j => j["avg"] = average(j.gradsList));
    return arr;
}
...
function studentsResult(arr) {
    let newArray = gradesAvg(arr);
    newArray.map(j => (j.avg >= 50 ? j['result'] = 'Passed' : j['result'] = "Failed"))
    return newArray;
}
